...
const MarryDate = () => {
    ...
    useEffect(() => {
        const fetchMarryDate = async () => {
            try {
                const response = await axios.get(`http://172.30.1.9:8085/marrydate/${userSeq}`);
                if (response.data) {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            if (hasMarryDate) {
                ...
                response = await axios.post(`http://172.30.1.9:8085/marrydate/update`, {
                    marryDt: marryDate,
                    memberSeq: userSeq,
                });
            } else {
                // 등록된 결혼예정일이 없으면 등록하기 호출
                response = await axios.post(`http://172.30.1.9:8085/marrydate`, {
                    marryDt: marryDate,
                    memberSeq: userSeq,
                });
            }
            ...
            if (response.status === 200) {
                ...
                window.location.href = "http://172.30.1.9:3000";
            } else {
