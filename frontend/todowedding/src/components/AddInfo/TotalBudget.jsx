...
const TotalBudget = () => {
    ...
    useEffect(() => {
        const fetchTotalBudget = async () => {
            try {
                //백엔드로 총 예산 조회 요청 보내기
                const response = await axios.get(`http://172.30.1.9:8085/totalbudget/select/${userSeq}`);
                if (response.data) {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            if (hasTotalBudget) {
                ...
                response = await axios.post("http://172.30.1.9:8085/totalbudget/update", {
                    total_budget: totalBudget,
                    member_seq: userSeq,
                });
            } else {
                //등록된 총 예산이 없는 경우 등록 API 호출하기
                response = await axios.post("http://172.30.1.9:8085/totalbudget/insert", {
                    total_budget: totalBudget,
                    member_seq: userSeq,
                });
            }
