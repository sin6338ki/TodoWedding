...
public interface ChatMapper {
	...
	@Select("SELECT tc.chat_room_seq, tc.member_seq, tc.partner_seq, tm.nickname, MAX(tc2.chatting_create_dt) as last_date FROM (SELECT chat_room_seq, member_seq, partner_seq FROM tw_chatroom WHERE partner_seq=#{partnerSeq} ) AS tc INNER JOIN tw_member tm ON tc.member_seq = tm.member_seq INNER JOIN tw_chatting tc2 ON tc.chat_room_seq = tc2.chat_room_seq GROUP BY tc.chat_room_seq ORDER BY MAX(tc2.chatting_create_dt) DESC")
